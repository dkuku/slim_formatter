defmodule SlimFormatter.Diff do
  @moduledoc false
  ...
  defp container_to_algebra(open, list, close, diff_wrapper, item_to_algebra) do
    docs =
      ...
      |> Enum.reduce([], fn
        {:doc_cons, {:doc_cons, _, str1}, str2}, [head | acc]
        when is_integer(head) and is_binary(str1) and is_binary(str2) ->
          [head + 1 | acc]

        {:doc_cons, {:doc_cons, _, str1}, str2}, acc when is_binary(str1) and is_binary(str2) ->
          [1 | acc]

        other, [head | acc] when is_integer(head) and is_binary(other) ->
          [head + 1 | acc]
      ...
      |> Enum.reduce([], fn
        el, acc when is_integer(el) -> [String.duplicate(".", el) | acc]
        el, acc -> [el | acc]
      end)
